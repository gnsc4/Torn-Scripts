// ==UserScript==
// @name         Torn: Racing enhancements (Modified for HTML Data) - SIMPLIFIED
// @namespace    lugburz.racing_enhancements
// @version      0.6.2 (HTML Data Extraction - DOM Fix - NO AJAX LIB)
...
(function() {
    ...
    console.log("Script Injected - HTML Data Support (AJAX Interception REMOVED)");

    // **NEW: Try to extract raceData from HTML**
    const htmlRaceData = extractRaceDataFromHTML();

    if (htmlRaceData) {
        console.log("Race data found in HTML source!");
        parseRacingData(htmlRaceData); // Process data directly from HTML
    } else {
        console.log("Race data NOT found in HTML source. AJAX fallback REMOVED in this version.");
        // No AJAX fallback in this simplified version
    }


    // Configuration constants - moved to the top for better visibility
    ...
    function showSpeed() {
        ...
        x = setInterval(function() {
            ...
            let laps = $('#racingupdatesnew').find('div.title-black').text().split(" - ")[1].split(" ")[1]; // Corrected index to [1]
    ...
    function parseRacingData(data) {
        ...
        // Log the status and timeEnded to see what the API is sending
        console.log("Race ID:", data.raceID);
        console.log("timeData.status:", data.timeData ? data.timeData.status : "timeData is undefined");
        console.log("timeData.timeEnded:", data.timeData ? data.timeData.timeEnded : "timeData is undefined");
        console.log("data.timeData:", data.timeData);


        // calc, sort & show race results
        if (data.timeData && data.timeData.status >= 3) {
            console.log("Race status detected as >= 3, proceeding with results.");
            ...
            console.log("Race status NOT >= 3 (or timeData missing), race considered in progress.");
        }
    }
    ...
    function addSettingsDiv() {
        if ($("#racingupdatesnew").size() > 0 && $('#racingEnhSettings').size() < 1) {
            ...
            settingsDiv.style.lineHeight = '24px';
            settingsDiv.style.paddingLeft = '10px';
            settingsDiv.style.paddingRight = '10px';
            settingsDiv.style.background = 'repeating-linear-gradient(90deg,#242424,#242424 2px,#2e2e2e 0,#2e2e2e 4px)';
            settingsDiv.style.borderRadius = '5px';

            // Create the Settings link
            const settingsLink = document.createElement('a');
            settingsLink.id = 'racingEnhSettings';
            settingsLink.style.textAlign = 'right';
            settingsLink.style.cursor = 'pointer';
            settingsLink.textContent = 'Settings';
            settingsDiv.appendChild(settingsLink);

            // Create the settings container div
            const settingsContainer = document.createElement('div');
            settingsContainer.id = 'racingEnhSettingsContainer';
            settingsContainer.style.display = 'none';
            settingsDiv.appendChild(settingsContainer);

            // Create the settings UL
            const settingsUL = document.createElement('ul');
            settingsUL.style.color = '#ddd';
            settingsContainer.appendChild(settingsUL);

            // Settings list items (reusing your original settings list structure)
            const settingsListItems = [
                { id: 'showSpeedChk', label: 'Show current speed' },
                { id: 'showNotifChk', label: 'Show notifications' },
                { id: 'showResultsChk', label: 'Show results' },
                { id: 'showSkinsChk', label: 'Show racing skins' },
                { id: 'showPositionIconChk', label: 'Show position icons' },
                {
                    html: '<label>Fetch racing skill from the API (<a href="https://www.torn.com/preferences.php#tab=api">link to your API key</a>)</label><span class="input-wrap" style="margin: 0px 5px 5px;">' +
            ...
            settingsListItems.forEach(item => {
                ...
                if (item.html) {
                    listItem.innerHTML = item.html; // For the API key setting with complex HTML
                } else {
                ...
                settingsUL.appendChild(listItem);
            });

            $('#racingupdatesnew').prepend(settingsDiv); // Prepend the constructed div

            // --- Event listeners and value setting (rest of your original function) ---
...
${currentPoints ? currentPoints : 'N/A'} / Daily gain: ${currentPoints && oldPoints ? 1*currentPoints - 1*oldPoints : 'N/A'}
    ...
    function extractRaceDataFromHTML() {
        // 1. Get all script tags on the page
        const scriptTags = document.querySelectorAll('script');

        // 2. Iterate through the script tags
        for (const scriptTag of scriptTags) {
            const scriptContent = scriptTag.textContent;

            // 3. Check if the script content contains 'var raceData ='
            if (scriptContent.includes('var raceData =')) {
                try {
                    // 4. Use a regular expression to extract the JSON string
                    const raceDataMatch = scriptContent.match(/var raceData = ({.*?});/s);
                    if (raceDataMatch && raceDataMatch[1]) {
                        const raceDataJSON = raceDataMatch[1];

                        // 5. Parse the JSON string into a JavaScript object
                        const parsedRaceData = JSON.parse(raceDataJSON);
                        return parsedRaceData; // Return the parsed object
                    }
                } catch (error) {
                    console.error("Error parsing raceData from HTML:", error);
                    return null; // Handle parsing errors gracefully
                }
            }
        }

        return null; // raceData not found in HTML
    }
