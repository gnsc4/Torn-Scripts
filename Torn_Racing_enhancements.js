...
// Add this right after the constants section
...
function debugLog(message, data = null) {
    ...
    const lineNumber = stack.match(/\:(\d+)\:\d+\)?$/)[1];
    const logMessage = `[DEBUG][${timestamp}][Line ${lineNumber}] ${message}`;
    if (data) {
...
async function updateDriversList() {
    debugLog('Starting updateDriversList');
    const driversList = document.getElementById('leaderBoard');
    
    debugLog('Drivers list element:', {
        exists: !!driversList,
        id: driversList?.id,
        childCount: driversList?.childNodes?.length
    });

    if (updating || driversList === null) {
        debugLog('Skipping updateDriversList - already updating or no drivers list', {
            updating,
            driversListExists: !!driversList
        });
        return;
    }

    FETCH_RS = !!(GM_getValue('apiKey') && GM_getValue('apiKey').length > 0);
    debugLog('FETCH_RS status:', FETCH_RS);

    watchForDriversListContentChanges(driversList);

    const driverIds = getDriverIds(driversList);
    debugLog('Retrieved driver IDs:', driverIds);

    if (!driverIds || !driverIds.length) {
        debugLog('No driver IDs found, returning');
        return;
    }

    updating = true;
    debugLog('Starting drivers update process');
    ...
    debugLog('Retrieved racing skills:', racingSkills);
    
    const racingSkins = SHOW_SKINS ? await getRacingSkinOwners(driverIds) : {};
    debugLog('Retrieved racing skins:', racingSkins);
...
function getDriverIds(driversList) {
    debugLog('getDriverIds called with driversList:', {
        exists: !!driversList,
        nodeType: driversList?.nodeType,
        childCount: driversList?.childNodes?.length
    });

    if (!driversList) {
        debugLog('ERROR: driversList is null/undefined');
        return [];
    }

    const driverItems = driversList.querySelectorAll('.driver-item');
    debugLog('Found driver items:', {
        count: driverItems?.length,
        items: Array.from(driverItems).map(d => d.innerHTML?.substring(0, 50))
    });

    const driverIds = Array.from(driverItems)
        .map(driver => {
            try {
                const id = getDriverId(driver);
                debugLog('Extracted driver ID:', { driver: driver.innerHTML?.substring(0, 50), id });
                return id;
            } catch (err) {
                debugLog('Failed to get driver ID:', { error: err.message, driver: driver.innerHTML?.substring(0, 50) });
                return null;
            }
        })
        .filter(id => id !== null);

    debugLog('Final driver IDs:', { count: driverIds.length, ids: driverIds });
    return driverIds;
}

function getDriverId(driverUl) {
    debugLog('getDriverId called', { element: driverUl?.outerHTML?.substring(0, 100) });
    
    if (!driverUl) {
        debugLog('ERROR: driverUl is null/undefined');
        return null;
    }

    const liElement = driverUl.closest('li');
    if (!liElement) {
        debugLog('ERROR: Could not find parent li element', { driverHtml: driverUl.outerHTML?.substring(0, 100) });
        return null;
    }

    const id = liElement.id;
    if (!id || !id.startsWith('lbr-')) {
        debugLog('ERROR: Invalid ID format', { id, driverHtml: driverUl.outerHTML?.substring(0, 100) });
        return null;
    }

    const numberId = +id.substr(4);
    if (isNaN(numberId)) {
        debugLog('ERROR: ID is not a number', { id, numberId });
        return null;
    }

    debugLog('Successfully extracted driver ID', { id, numberId });
    return numberId;
}
...
function fetchRacingSkillForDrivers(driverIds) {
    debugLog('Fetching racing skill for drivers', {function: 'fetchRacingSkillForDrivers', driverIds});
    const apiKey = GM_getValue('apiKey');
    
    return new Promise((resolve, reject) => {
        debugLog('Making API request for racing skills', {url: `api.torn.com/user/${driverIds}`});
        GM_xmlhttpRequest({
            method: 'POST',
            ...
            onload: (response) => {
                try {
                    ...
                    debugLog('Error parsing API response:', err);
                    reject(err);
                }
            },
            onerror: (err) => {
                debugLog('API request failed:', err);
                reject(err);
            }
        });
    });
...
function showSpeed() {
    debugLog('Starting showSpeed function', {function: 'showSpeed'});
    
    if (!SHOW_SPEED || $('#racingdetails').size() < 1 || $('#racingdetails').find('#speed_mph').size() > 0) {
        debugLog('Skipping showSpeed - conditions not met', {
            SHOW_SPEED,
            hasRacingDetails: $('#racingdetails').size() > 0,
            hasSpeedMph: $('#racingdetails').find('#speed_mph').size() > 0
        });
        return;
    }

    // Check completion format to detect completed races
    const completion = $('#racingdetails').find('li.pd-completion').text().trim();
    debugLog('Checking completion format:', { completion });
    
    if (completion.includes(':')) {
        debugLog('Found time-based completion, race is completed');
        // Store completion time for reference
        window.racing = window.racing || {};
        window.racing.completionTime = completion;
        processCompletedRace();
        maybeClear();
        return;
    }

    // First check if this is a completed race
    const status = window?.racing?.data?.timeData?.status;
    debugLog('Race status check:', { status });
    
    if (status >= 3) {
        debugLog('Race is completed, not showing speed');
        maybeClear();
        return;
    }
    
    // Also check if there's a completed race result visible
    if ($('#racingupdatesnew').find('.race-results').length > 0) {
        debugLog('Race results found, assuming completed race');
        maybeClear();
        return;
    }

    debugLog('Setting up speed display elements');
    // save some space
    ...
    x = setInterval(function() {
        if ($('#racingupdatesnew').find('div.track-info').size() < 1) {
            debugLog('No track info found, clearing interval');
            maybeClear();
            return;
        }
        ...
        debugLog(`Speed calculation data - Laps: ${laps}, Length: ${len}, Completion: ${compl}`);

        if (last_compl >= 0) {
            let speed = (compl - last_compl) / 100 * laps * len * 60 * 60 * 1000 / period;
            debugLog(`Calculated speed: ${speed.toFixed(2)}mph`);
...
function getRaceIdFromPage() {
    debugLog('Attempting to get race ID from multiple sources');

    const sources = [
        // From track-wrap container (most reliable)
        () => {
            const trackWrap = $('.track-wrap');
            if (trackWrap.length) {
                const raceId = trackWrap.attr('data-race') || trackWrap.attr('data-id');
                if (raceId) {
                    debugLog('Found race ID from track-wrap', { raceId, element: trackWrap.prop('outerHTML') });
                    return raceId;
                }
            }
            return null;
        ...
        // From track info container
        () => {
            const trackInfo = $('.track-info');
            if (trackInfo.length) {
                const parentWithRace = trackInfo.closest('[data-race], [data-id]');
                if (parentWithRace.length) {
                    const raceId = parentWithRace.attr('data-race') || parentWithRace.attr('data-id');
                    debugLog('Found race ID from track info parent', { raceId, element: parentWithRace.prop('outerHTML') });
                    return raceId;
                }

                // Try direct track info container
                const trackRaceId = trackInfo.attr('data-race') || trackInfo.attr('data-id');
                if (trackRaceId) {
                    debugLog('Found race ID from track info', { raceId: trackRaceId });
                    return trackRaceId;
                }
            }
            return null;
        ...
        // From drivers list container
        () => {
            const driversList = $('.drivers-list');
            if (driversList.length) {
                const raceId = driversList.attr('data-race') || driversList.closest('[data-race]').attr('data-race');
                if (raceId) {
                    debugLog('Found race ID from drivers list', { raceId });
                    return raceId;
                }
            }
            return null;
        ...
        () => {
            ...
            if (raceId) {
                debugLog('Found race ID from URL', { raceId });
                return raceId;
            }
            return null;
        },

        // From racing data global object
        () => {
            const raceData = window?.racing?.data;
            if (raceData?.raceID) {
                debugLog('Found race ID from racing data', { raceId: raceData.raceID });
    ...
    // Try each source in order until we find a race ID
    let raceId = null;
    for (const source of sources) {
        try {
            raceId = source();
            if (raceId) {
                debugLog('Successfully found race ID', { raceId, source: source.toString() });
                break;
            }
        } catch (e) {
            debugLog('Error trying source', { error: e.message, source: source.toString() });
        }
    }
...
function processCompletedRace() {
    debugLog('Processing completed race');
    try {
        // Initialize retry count
        if (!window.raceIdRetries) {
            window.raceIdRetries = 0;
        }

        const raceId = getRaceIdFromPage();
        
        if (!raceId && window.raceIdRetries < 3) {  // Try up to 3 times
            debugLog('Could not find race ID, retrying in 500ms', { attempt: window.raceIdRetries + 1 });
            window.raceIdRetries++;
            
            // Add a retry mechanism for race ID with exponential backoff
            setTimeout(() => {
                processCompletedRace();
            }, 500 * Math.pow(2, window.raceIdRetries - 1));
            return;
        }

        if (!raceId) {
            debugLog('Failed to find race ID after all retries');
            return;
        }
        ...
        RACE_ID = raceId;
        debugLog('Found race ID:', { raceId });
        processRaceResults(raceId);
    } catch (e) {
        debugLog('Error processing completed race:', e);
    }
}

function processRaceResults(raceId) {
    debugLog('Processing race results', { raceId });

    // Add race link if not already present
    if ($('#raceLink').size() < 1) {
    ...
    // Use stored race data if available
    const data = window?.racing?.data;
    debugLog('Race data found:', { hasData: !!data, hasRaceData: !!data?.raceData });
    
    if (data?.raceData) {
        // Get user name
        const userNameEl = $("#sidebarroot").find("a[class^='menu-value']");
        const my_name = userNameEl.length ? userNameEl.text().trim() : data.user?.playername;
        debugLog('Processing race data', { my_name });
        
        try {
            processRaceData(data, my_name, raceId);
        } catch (e) {
            debugLog('Error processing race data:', e);
        }
    }
}

function processRaceData(data, my_name, raceId) {
    debugLog('Starting race data processing', { 
        hasCars: !!data.raceData.cars,
        hasCarInfo: !!data.raceData.carInfo,
        raceId
    });

    const results = [], crashes = [];

    if (!data.raceData.cars || !data.raceData.carInfo) {
        debugLog('Missing required race data');
        return;
    }
    ...
    try {
        ...
        debugLog('Processing race entries', {
            playerCount: Object.keys(carsData).length,
            trackIntervals,
            totalLaps
        });
        ...
        for (const playername in carsData) {
            ...
            if (intervals.length / trackIntervals == totalLaps) {
                // Player finished race
                let raceTime = 0, bestLap = 9999999999;
        ...
        debugLog('Race results processed', { 
            finishers: results.length,
            crashes: crashes.length
        });

        // Generate CSV and add export button
        addExportButton(results, crashes, my_name, raceId, data.timeData.timeEnded);
        ...
        debugLog('Error in processRaceData:', e);
    }
}
...
function parseRacingData(data) {
    debugLog('Starting parseRacingData', {function: 'parseRacingData', data: {
        raceID: data?.raceID,
    ...
    if (!data || !data.timeData) {
        debugLog('No race data available');
        return;
    }

    const raceStatus = data.timeData.status;
    debugLog('Race status:', { status: raceStatus });
    
    // Store race status and data globally
    window.racing = window.racing || {};
    window.racing.data = data;  // Store entire data object
    window.racing.data.timeData = window.racing.data.timeData || {};
    window.racing.data.timeData.status = raceStatus;

    // Always update these regardless of race status
    ...
    // Get completion time once
    const hasCompletionTime = $('#racingdetails').find('li.pd-completion').text().trim().includes(':');
    
    // If race is already complete or has completion time, process results
    if (raceStatus >= 3 || hasCompletionTime) {
        debugLog('Processing completed race', { raceStatus, hasCompletionTime });
        if (data.raceID) {
            RACE_ID = data.raceID;  // Set RACE_ID before processing
        }
        processCompletedRace();
        return;
    }
...
function initializeScript() {
    // Event handlers for loader page
    ajax((page, xhr) => {
        ...
        try {
            ...
            if (data.timeData && data.timeData.status >= 3) {
                processCompletedRace();
            }
        } catch (e) {
            debugLog('Error in ajax callback:', e);
        }
