...
// @version      1.2.4
// @description  Alerts when no booster cooldown is active, adds Quick Use panel, and allows customization of booster colors and categories. Also includes faction armoury booster support.
// @author       GNSC4 [268863]
...
(function() {
    ...
    GM_addStyle(`
        ...
        .happy-item { border-left: 3px solid #FFEB3B; color: white; /* Corrected color */ }
        ...
        if (!gui) {
            ...
            if (factionBoostersCheckbox) {
                factionBoostersCheckbox.addEventListener('change', function() {
                    ...
                    addBoosterQuickUseButtons(); // Refresh quick use visibility based on new setting
                });
            }
    ...
    function showBoosterCustomizationUI(currentBoosters) {
        ...
        customizationUI.querySelector('.booster-customization-button.save').addEventListener('click', () => {
             ...
             const finalBoostersCorrected = currentItemsInUI
                 .filter(item => !item.hasAttribute('data-remove')) // Filter out items marked for removal
                 .map(item => {
                     const boosterId = parseInt(item.dataset.boosterId);
                     // Find the corresponding data (including potentially updated color) from the selectedBoosters array
                     const boosterData = selectedBoosters.find(b => b.id === boosterId);
                     const colorPicker = item.querySelector('input[type="color"]');
                     return {
                         id: boosterId,
                         name: boosterData ? boosterData.name : 'Unknown', // Get name from original data
                         color: colorPicker ? colorPicker.value : (boosterData ? boosterData.color : getDefaultBoosterColor(boosterId)) // Get latest color
                     };
                 });
             // --- End Corrected Save Logic ---
             localStorage.setItem('customQuickUseBoosters', JSON.stringify(finalBoostersCorrected)); // Save the corrected list
             customizationUI.remove();
             rebuildQuickUsePanel(); // Use rebuild function instead of add function
    ...
    // --- NEW Function to specifically rebuild the quick use panel ---
    function rebuildQuickUsePanel() {
        const existingContainer = document.querySelector('.booster-quick-use-container');
        if (existingContainer) { existingContainer.remove(); }

        // debugLog('Rebuilding Quick Use UI panel.');
        const quickUseContainer = document.createElement('div'); quickUseContainer.className = 'booster-quick-use-container';
        const savedQuickUseBoosters = localStorage.getItem('customQuickUseBoosters'); let quickUseBoosters = []; const defaultQuickBoosters = [ { id: 532, name: "Can of Red Cow" }, { id: 533, name: "Can of Taurine Elite" }, { id: 530, name: "Can of Munster" } ];
        if (savedQuickUseBoosters) { try { quickUseBoosters = JSON.parse(savedQuickUseBoosters); quickUseBoosters.forEach(b => { if (!b.color) b.color = getDefaultBoosterColor(b.id); }); } catch (e) { console.error("Booster Alerts: Error parsing saved quick use boosters", e); quickUseBoosters = defaultQuickBoosters.map(b => ({ ...b, color: getDefaultBoosterColor(b.id) })); localStorage.setItem('customQuickUseBoosters', JSON.stringify(quickUseBoosters)); } } else { quickUseBoosters = defaultQuickBoosters.map(b => ({ ...b, color: getDefaultBoosterColor(b.id) })); localStorage.setItem('customQuickUseBoosters', JSON.stringify(quickUseBoosters)); }
        ...
        // Set initial visual state, position relies on later adjustment calls
        boosterButtons.forEach(btn => { btn.style.display = isMinimized ? 'none' : 'block'; }); settingsButton.style.display = isMinimized ? 'none' : 'block'; quickUseContainer.style.padding = isMinimized ? '2px' : '10px'; toggleButton.textContent = isMinimized ? '+' : 'X';
        adjustBoosterQuickUsePosition(); // Adjust position after rebuilding
    }
