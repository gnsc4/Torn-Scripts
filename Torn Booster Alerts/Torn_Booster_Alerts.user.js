...
// @version      1.2.2
// @description  Alerts when no booster cooldown is active, adds Quick Use panel for easy access to boosters, and allows customization of booster colors and categories. Also includes faction armoury booster support.
// @author       GNSC4 [268863]
...
(function() {
    ...
    function addBoosterQuickUseButtons() {
        ...
        // Determine if the Quick Use UI should be shown based on page and faction setting
        // Show on Items page ONLY if useFactionBoosters is FALSE
        // Show on Faction page ONLY if useFactionBoosters is TRUE
        const shouldShowQuickUse = (isItemsPage && !useFactionBoosters) || (isFactionArmouryBoostersPage && useFactionBoosters);

        // Remove existing container if it somehow exists
        ...
        if (!shouldShowQuickUse) {
            // debugLog('Quick Use UI not shown (Page/Setting condition not met).');
            return; // Exit the function, don't create the UI
        }
