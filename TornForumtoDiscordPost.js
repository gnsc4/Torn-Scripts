...
function setupObserver() {
    ...
    observer = new MutationObserver(async (mutations) => {
        ...
        // We only care about new posts being added.
        let newPostAdded = mutations.some(mutation => {
            return Array.from(mutation.addedNodes).some(node => 
                node.nodeType === Node.ELEMENT_NODE && node.classList.contains('post-container')
            );
        });
    
        if (newPostAdded) {
            logDebug("New post detected, updating extraction data.");
            try {
                // Update data for the current forum
                const currentForumUrl = window.location.href;
                const existingForumIndex = extractedData.findIndex(data => data.forumUrl === currentForumUrl);
    
                if (existingForumIndex !== -1) {
                    // Extract new posts and update data
                    const newExtractedData = await extractPosts();
                    extractedData[existingForumIndex].posts = newExtractedData;
                    extractedData[existingForumIndex].timestamp = Date.now();
    
                    // Reformat and update the copy button
                    const discordFormattedPosts = await formatForDiscord(newExtractedData);
                    addCopyButton(discordFormattedPosts);
                }
            } catch (error) {
                logError("Error updating extraction data:", error);
                displayNotification(`Error updating extraction data: ${error.message}`, true);
            }
        }
    });
