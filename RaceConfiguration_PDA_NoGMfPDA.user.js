...
(function() {
    ...
    const guiHTML = `
        <div id="raceConfigGUI" style="position: fixed; top: 75px; left: 20px; background-color: #333; border: 1px solid #666; padding: 15px; z-index: 1000; border-radius: 5px; color: #eee;">
            <h3 style="margin-top: 0; color: #fff;">Race Config - DEBOUNCE DEBUG v2.57</h3>
            ...
            <div style="border-top: 1px solid #eee; padding-top: 10px;">
                <h4>Presets (DEBUG v2.57 - Debounced Save)</h4>
            ...
            <span style="font-size: 0.8em; color: #999; position: absolute; bottom: 5px; right: 5px;">v2.57 - Debounced Save Debug</span>
        </div>
    `;
    $('body').append(guiHTML);
    ...
    function saveApiKey() {
        ...
        alert('API Key Saved (v2.57 - Debounced Debug)');
    ...
    function loadPresets() {
        alert("loadPresets() - START (v2.57)"); // DEBUG ALERT
        ...
        alert("loadPresets() - END (v2.57)"); // DEBUG ALERT
    }

    function savePreset() {
        alert("savePreset() - START (v2.57) - DEBOUNCED FUNCTION"); // DEBUG ALERT - ALERT AT START OF FUNCTION!
        const presetName = prompt("Enter a name for this preset (v2.57 DEBUG - DEBOUNCED):");
        alert("savePreset() - After prompt, presetName: " + presetName); // DEBUG ALERT
        if (!presetName) {
            alert("savePreset() - No preset name, cancelled (v2.57)"); // DEBUG ALERT
            return;
        }

        const presets = GM_getValue(PRESET_STORAGE_KEY, {});
        alert("savePreset() - Before save, current presets: " + JSON.stringify(presets)); // DEBUG ALERT
        presets[presetName] = { name: presetName, debugVersion: "v2.57-Debounced" }; // Simplified preset data
        ...
        alert("savePreset() - After loadPresets() call (v2.57)"); // DEBUG ALERT
        alert("savePreset() - END (v2.57) - DEBOUNCED FUNCTION"); // DEBUG ALERT
    }

    function removePreset(presetName, buttonElement) {
        alert("removePreset() - START, presetName: " + presetName + " (v2.57)"); // DEBUG ALERT
        if (confirm(`Are you sure you want to delete preset "${presetName}"? (v2.57 DEBUG - DEBOUNCED)`)) {
            alert("removePreset() - Confirmed delete: " + presetName + " (v2.57)"); // DEBUG ALERT
            const presets = GM_getValue(PRESET_STORAGE_KEY, {});
            delete presets[presetName];
            GM_setValue(PRESET_STORAGE_KEY, presets);
            $(buttonElement).closest('.preset-button-container').remove();
            alert("removePreset() - Preset removed from GUI: " + presetName + " (v2.57)"); // DEBUG ALERT
        } else {
            alert("removePreset() - Cancelled delete: " + presetName + " (v2.57)"); // DEBUG ALERT
        }
        alert("removePreset() - END, presetName: " + presetName + " (v2.57)"); // DEBUG ALERT
    }

    function applyPreset(presetConfig) {
        alert("applyPreset() - Applying preset: " + presetConfig.name + " (v2.57)"); // DEBUG ALERT
        alert("applyPreset() - END (v2.57)"); // DEBUG ALERT - In simplified version, applyPreset does nothing else
    }
    ...
    function createPresetButton(presetName, presetConfig) {
        ...
        const button = $(`<button class="preset-button" style="cursor: pointer; margin-right: 2px; color: #ddd; background-color: #555; border: 1px solid #777; border-radius: 3px; padding: 8px 15px;">${presetName} (v2.57)</button>`); // v2.57 label
    ...
    function setupEventListeners() {
        ...
        $('#savePresetButton').on('click', debounce(savePreset, 500)); // DEBOUNCED savePreset! - 500ms delay
    ...
    $(document).ready(function() {
        if ($('div.content-title > h4').length > 0 && !$('#toggleRaceGUIButton').length) {
            const toggleButton = $(`<button id="toggleRaceGUIButton" style="color: #ddd; background-color: #555; border: 1px solid #777; border-radius: 3px; padding: 8px 15px; cursor: pointer; text-decoration: none; margin-right: 10px;">Race Config GUI (v2.57)</button>`); // v2.57 Label
            ...
            $('div.content-title > h4').append('<span style="color: orange; margin-left: 10px;">v2.57 - DEBOUNCED SAVE DEBUG - PRESETS ONLY</span>'); // Orange - debounced debug
    });
