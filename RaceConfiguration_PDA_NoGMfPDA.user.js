...
(function() {
    ...
    function createRaceConfigGUI() {
        ...
        gui.innerHTML = `
            ...
            <div style="text-align: center; margin-top: 20px; color: #888; font-size: 1.2em;">
                ...
                <a href="https://www.torn.com/forums.php#/p=threads&f=67&t=16454445&b=0&a=0" target="_blank" style="color: #888; text-decoration: none;">v3.5.7 Official Forum Link</a>
            </div>
        `;
    ...
    function initializeGUI(gui) {
        // First, ensure car data is loaded
        ensureCarDataLoaded().then(() => {
            loadApiKey();
            populateTimeDropdowns();
            loadPresets();
        
            // Display presets after ensuring car data is loaded
            displayPresets();
        ...
        // Set up event listeners immediately (don't wait for car data)
        ...
        // Ensure car data is loaded and then update displays
        (async function() {
            await ensureCarDataLoaded();
            displayPresets();
            updateQuickPresetsDisplay();
            updateQuickLaunchButtons();
        })();
    ...
    function toggleRaceGUI() {
        ...
        if (gui) {
            ...
            // Ensure car data is loaded when GUI is opened
            if (isVisible) {
                ensureCarDataLoaded().then(() => {
                    // Update dropdowns and buttons after car data is loaded
                    displayPresets();
                    updateQuickPresetsDisplay();
                    updateQuickLaunchButtons();
                    
                    // Update car UI elements
                    if (document.getElementById('carDropdown')) {
                        syncCarDropdownsSafe();
                    }
                });
            }
        } else {
            ...
            // Ensure car data is loaded for new GUI
            ensureCarDataLoaded();
        }
    }
    ...
    function saveAutoJoinPreset() {
        ...
        const preset = {
            track: document.getElementById('autoJoinTrack')?.value || 'all',
            ...
            hideBets: document.getElementById('hideBets')?.checked || false
        };
    ...
    async function applyAutoJoinPreset(preset) {
        try {
            // Ensure car data is loaded first
            await ensureCarDataLoaded();
    ...
    async function startAutoJoin() {
        try {
            ...
            console.error('Error in startAutoJoin:', error);
            displayStatusMessage('Error starting auto-join', 'error');
        }
    }
    ...
    // Add a function to preload car data
    async function ensureCarDataLoaded() {
        console.log('Racing Config: Ensuring car data is loaded');
        
        // Create a safer check function that won't throw errors if elements don't exist yet
        const checkCarData = () => {
            const carDropdown = document.getElementById('carDropdown');
            const autoJoinCarDropdown = document.getElementById('autoJoinCar');
            
            // If either dropdown doesn't exist or is empty
            if (!carDropdown || carDropdown.options.length <= 1 || 
                !autoJoinCarDropdown || autoJoinCarDropdown.options.length <= 1) {
                return false;
            }
            return true;
        };
        
        // If car data is already loaded, we're good
        if (checkCarData()) {
            console.log('Racing Config: Car data already loaded');
            return true;
        }
        
        console.log('Racing Config: Car data needs to be loaded');
        
        // Try to create car elements if they don't exist
        await createCarElementsIfNeeded();
        
        // Try to update car list
        try {
            await updateCarListSafe();
            return true;
        } catch (err) {
            console.warn('Racing Config: Error loading car data:', err);
            return false;
        }
    }
 
    // Add a function to create car elements if they don't exist
    async function createCarElementsIfNeeded() {
        const raceConfigGUI = document.getElementById('raceConfigGUI');
        
        // If the GUI doesn't exist yet, create it offscreen
        if (!raceConfigGUI) {
            const tempGUI = createRaceConfigGUI();
            tempGUI.style.position = 'absolute';
            tempGUI.style.left = '-9999px';
            tempGUI.style.display = 'block';
            document.body.appendChild(tempGUI);
            await new Promise(resolve => setTimeout(resolve, 100));
        }
    }
 
    // Add a safer version of updateCarList that won't throw errors
    async function updateCarListSafe() {
        return new Promise(async (resolve, reject) => {
            try {
                // Wait a bit to ensure DOM is ready
                await new Promise(res => setTimeout(res, 200));
                
                // Try to find car-related elements
                const carDropdown = document.getElementById('carDropdown');
                const carStatusMessage = document.getElementById('carStatusMessage');
                const updateCarsButton = document.getElementById('updateCarsButton');
                ...
                if (!carDropdown) {
                    console.log('Racing Config: Car dropdown not found, creating fallback elements');
                    // Create temporary elements if needed
                    const tempContainer = document.createElement('div');
                    tempContainer.style.display = 'none';
                    tempContainer.innerHTML = `
                        <select id="carDropdown"><option value="">Select a car...</option></select>
                        <select id="autoJoinCar"><option value="">Select a car...</option></select>
                        <div id="carStatusMessage"></div>
                        <button id="updateCarsButton">Update Cars</button>
                    `;
                ...
                const apiKey = GM_getValue(STORAGE_API_KEY, '');
                if (!apiKey) {
                    console.log('Racing Config: No API key found');
                    resolve(false);
                    return;
                }
                        ...
                        try {
                            if (response.status === 200) {
                                ...
                                if (data.error) {
                                    ...
                                    populateCarDropdownSafe(data.enlistedcars);
                                    
                                    // Add a short delay then force sync car dropdowns
                                    setTimeout(() => {
                                        syncCarDropdownsSafe();
                                        console.log('Racing Config: Cars updated successfully');
                                        resolve(true);
                                    }, 500);
                                } else {
    ...
    // Add a safer version of populateCarDropdown
    function populateCarDropdownSafe(cars) {
        try {
            const dropdownIds = ['carDropdown', 'autoJoinCar'];
            ...
            window.carData = sortedCars;
            
            // Update each dropdown separately
            dropdownIds.forEach(id => {
                const dropdown = document.getElementById(id);
                if (!dropdown) {
                    console.log(`Racing Config: Dropdown ${id} not found`);
                    return;
                }
                ...
                console.log(`Racing Config: Added ${sortedCars.length} cars to dropdown ${id}`);
            });
            
            console.log('Racing Config: Car dropdowns populated successfully');
        } catch (error) {
            console.error('Racing Config: Error in populateCarDropdown:', error);
        }
    }
 
    // Add a safer version of syncCarDropdowns
    function syncCarDropdownsSafe() {
        try {
            const mainCarDropdown = document.getElementById('carDropdown');
            const autoJoinCarDropdown = document.getElementById('autoJoinCar');
            
            if (!mainCarDropdown || !autoJoinCarDropdown) {
                console.log('Racing Config: One or both dropdowns not found for sync');
                return false;
            }
            
            if (mainCarDropdown.options.length <= 1) {
                // Try to populate from stored car data
                if (window.carData && window.carData.length > 0) {
                    populateCarDropdownSafe({ cars: window.carData });
                    return true;
                }
                console.log('Racing Config: Main dropdown has no car options');
                return false;
            }
            ...
            try {
                ...
                // Clone options from main dropdown to auto-join dropdown
                autoJoinCarDropdown.innerHTML = mainCarDropdown.innerHTML;
                ...
                console.error('Racing Config: Error synchronizing dropdowns:', error);
                return false;
            }
        } catch (error) {
            console.error('Racing Config: Error in syncCarDropdownsSafe:', error);
            return false;
        }
    }
 
    // Call the modified init function when document is ready
    if (document.readyState === 'loading') {
