// ==UserScript==
// @name         Torn Race Config GUI - PDA & Desktop - v2.60 - Unique Key & Clear Presets
// @namespace    torn.raceconfiggui.pdadesktop
// @description  Simplified GUI with Unique Key & Clear Presets - v2.60 - Unique Key & Clear Presets
// @version      2.60-PDA-Desktop-GMfPDA-UniqueKeyClearPresets-DEBUG
...
(function() {
    ...
    const PRESET_STORAGE_KEY = 'torn.raceconfiggui.pdadesktop_racePresets_v2_60'; // <--- MORE UNIQUE PRESET STORAGE KEY (v2.60)
    ...
    const guiHTML = `
        <div id="raceConfigGUI" style="position: fixed; top: 75px; left: 20px; background-color: #333; border: 1px solid #666; padding: 15px; z-index: 1000; border-radius: 5px; color: #eee;">
            <h3 style="margin-top: 0; color: #fff;">Race Config - UNIQUE KEY DEBUG v2.60</h3>
            ...
            <div style="border-top: 1px solid #eee; padding-top: 10px;">
                <h4>Presets (DEBUG v2.60 - Unique Key & Clear)</h4>
            ...
            <span style="font-size: 0.8em; color: #999; position: absolute; bottom: 5px; right: 5px;">v2.60 - Unique Key & Clear Debug</span>
        </div>
    `;
    $('body').append(guiHTML);
    ...
    function saveApiKey() {
        ...
        alert('API Key Saved (v2.60 - Unique Key Debug)');
    ...
    function loadPresets() {
        console.log("loadPresets() - START (v2.60)"); // DEBUG CONSOLE LOG - START
        alert("loadPresets() - START (v2.60)"); // DEBUG ALERT - START
        ...
        console.log("loadPresets() - END (v2.60)"); // DEBUG CONSOLE LOG - END
        alert("loadPresets() - END (v2.60)"); // DEBUG ALERT - END
    }

    function savePreset() {
        alert("savePreset() - START (v2.60) - DEBOUNCED FUNCTION (1500ms)"); // DEBUG ALERT - START
        console.log("savePreset() - START (v2.60) - DEBOUNCED FUNCTION (1500ms)"); // DEBUG CONSOLE LOG - START
        const presetName = prompt("Enter a name for this preset (v2.60 DEBUG - Unique Key & Clear):"); // UPDATED PROMPT
        ...
        if (!presetName) {
            alert("savePreset() - No preset name, cancelled (v2.60)"); // DEBUG ALERT
            console.log("savePreset() - No preset name, cancelled (v2.60)"); // DEBUG CONSOLE LOG - CANCELLED
            return;
        }
        ...
        presets[presetName] = { name: presetName, debugVersion: "v2.60-UniqueKeyClear" }; // Simplified preset data - UPDATED VERSION
        ...
        alert("savePreset() - After loadPresets() call (v2.60)"); // DEBUG ALERT - AFTER LOADPRESETS CALL
        console.log("savePreset() - END (v2.60) - DEBOUNCED FUNCTION (1500ms)"); // DEBUG CONSOLE LOG - END
        alert("savePreset() - END (v2.60) - DEBOUNCED FUNCTION (1500ms)"); // DEBUG ALERT - END
    }

    function removePreset(presetName, buttonElement) {
        alert("removePreset() - START, presetName: " + presetName + " (v2.60)"); // DEBUG ALERT
        console.log("removePreset() - START, presetName: " + presetName + " (v2.60)"); // DEBUG CONSOLE LOG - START
        if (confirm(`Are you sure you want to delete preset "${presetName}"? (v2.60 DEBUG - Unique Key & Clear)`)) { // UPDATED CONFIRM MESSAGE
            alert("removePreset() - Confirmed delete: " + presetName + " (v2.60)"); // DEBUG ALERT
            console.log("removePreset() - Confirmed delete: " + presetName + " (v2.60)"); // DEBUG CONSOLE LOG - DELETE CONFIRMED
            const presets = GM_getValue(PRESET_STORAGE_KEY, {});
            delete presets[presetName];
            GM_setValue(PRESET_STORAGE_KEY, presets);
            $(buttonElement).closest('.preset-button-container').remove();
            alert("removePreset() - Preset removed from GUI: " + presetName + " (v2.60)"); // DEBUG ALERT
            console.log("removePreset() - Preset removed from GUI: " + presetName + " (v2.60)"); // DEBUG CONSOLE LOG - GUI REMOVED
        } else {
            alert("removePreset() - Cancelled delete: " + presetName + " (v2.60)"); // DEBUG ALERT
            console.log("removePreset() - Cancelled delete: " + presetName + " (v2.60)"); // DEBUG CONSOLE LOG - DELETE CANCELLED
        }
        alert("removePreset() - END, presetName: " + presetName + " (v2.60)"); // DEBUG ALERT
        console.log("removePreset() - END, presetName: " + presetName + " (v2.60)"); // DEBUG CONSOLE LOG - END
    }

    function applyPreset(presetConfig) {
        alert("applyPreset() - Applying preset: " + presetConfig.name + " (v2.60)"); // DEBUG ALERT
        console.log("applyPreset() - Applying preset: " + presetConfig.name + " (v2.60)"); // DEBUG CONSOLE LOG - APPLY START
        alert("applyPreset() - END (v2.60)"); // DEBUG ALERT - In simplified version, applyPreset does nothing else
        console.log("applyPreset() - END (v2.60)"); // DEBUG CONSOLE LOG - APPLY END
    }
    ...
    function createPresetButton(presetName, presetConfig) {
        ...
        const button = $(`<button class="preset-button" style="cursor: pointer; margin-right: 2px; color: #ddd; background-color: #555; border: 1px solid #777; border-radius: 3px; padding: 8px 15px;">${presetName} (v2.60)</button>`); // v2.60 label
    ...
    $(document).ready(function() {
        if ($('div.content-title > h4').length > 0 && !$('#toggleRaceGUIButton').length) {
            const toggleButton = $(`<button id="toggleRaceGUIButton" style="color: #ddd; background-color: #555; border: 1px solid #777; border-radius: 3px; padding: 8px 15px; cursor: pointer; text-decoration: none; margin-right: 10px;">Race Config GUI (v2.60)</button>`); // v2.60 Label
            ...
            $('div.content-title > h4').append('<span style="color: orange; margin-left: 10px;">v2.60 - UNIQUE KEY & CLEAR - PRESETS ONLY</span>'); // Orange - unique key & clear debug
    });
