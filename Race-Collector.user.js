...
// Add helper to wait for element readiness.
...
ajax((page, xhr) => {
    ...
    onElementReady('#racingupdatesnew', () => {
        addSettingsDiv();
        showSpeed();
    });
    onElementReady('#racingAdditionalContainer', showPenalty);

    if ($(location).attr('href').includes('sid=racing&tab=log&raceID=')) {
        onElementReady('#racingupdatesnew', addPlaybackButton);
    }
    try {
...
if ($(location).attr('href').includes('index.php')) {
    onElementReady('#mainContainer', displayDailyGains);
}
...
onElementReady('#racingupdatesnew', function() {
...
function extractData() {
    // ...existing code...
    Array.from(document.querySelectorAll('.yourSelector')).forEach(item => {
        const text = item.innerText; // or other property you are using
        // Add a guard so undefined values do not cause errors
        if (!text || typeof text.match !== 'function') return;
        const matchData = text.match(/yourRegexPattern/);
        // ...existing code using matchData...
    });
    // ...existing code...
}